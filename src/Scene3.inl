...
void Scene::init(int w, int h) {
	...
	geometry["steve"] = new Obj;
	geometry["cube"]->init();
	geometry["steve"]->init("models/steve.obj");
	...
	material["ceramic"]->shininess = 40.0f;

	material["diamond"] = new Material;
	material["diamond"]->ambient = vec4(0.1f, 0.1f, 0.3f, 1.0f);
	material["diamond"]->diffuse = vec4(0.551f, 0.949f, 0.886f, 1.0f);
	material["diamond"]->specular = vec4(0.551f, 0.949f, 0.886f, 1.0f);
	material["diamond"]->shininess = 90.0f;
	...
	model["steve"] = new Model;
	model["steve"]->geometry = geometry["steve"];
	model["steve"]->material = material["diamond"];
	...
	light["bulb"]->color = power * 3.0f * vec4(1.0f, 1.0f, 0.8f, 1.0f);
	...
	light["bulb2"]->color = power * 3.5f * vec4(1.0f, 1.0f, 0.8f, 1.0f);
	...
	light["bulb3"] = new SpotLight;
	light["bulb3"]->position = vec4(0.8f, 0.5f, 3.0f, 1.0f);
	light["bulb3"]->color = power * 1.0f * vec4(1.0f, 1.0f, 0.8f, 1.0f);
	light["bulb3"]->direction = vec4(-0.3f, 0.3f, -0.5f, 0.0f);
	light["bulb3"]->zNear = 0.1f;
	light["bulb3"]->zFar = 6.0f;
	light["bulb3"]->light_angle = 150.0f;
	//light["bulb3"]->ignore.insert(model["bulb"]);

	//node["world"]->models.push_back(model["bulb"]);
	//node["world"]->modeltransforms.push_back(translate(vec3(light["bulb3"]->position)) * scale(vec3(0.1f)));
	...
	node["corner"]->models.push_back(model["brick piece"]);
	node["corner"]->modeltransforms.push_back(translate(vec3(-1.25f, 2.0f, 0.0f)) * rotate(float(M_PI) / 4, vec3(0.0f, 1.0f, 0.0f)) * rotate(float(M_PI) / 2, vec3(1.0f, 0.0f, 0.0f)) * scale(vec3(6.0f, 0.1f, 8.0f)));
	node["corner"]->models.push_back(model["brick piece"]);
	node["corner"]->modeltransforms.push_back(translate(vec3(1.25f, 2.0f, 0.0f)) * rotate(float(M_PI) / 4, vec3(0.0f, 1.0f, 0.0f)) * rotate(float(M_PI) / 2, vec3(0.0f, 0.0f, 1.0f)) * scale(vec3(8.0f, 0.1f, 6.0f)));

	node["world"]->childnodes.push_back(node["corner"]);
	node["world"]->childtransforms.push_back(mat4(1.0f));
	node["world"]->models.push_back(model["steve"]);
	node["world"]->modeltransforms.push_back(scale(vec3(0.5f, 0.5f, 0.5f)) * translate(vec3(0.0f, 0.0f, 2.0f)) * rotate(float(M_PI) / 2, vec3(0.0f, 1.0f, 0.0f)));
